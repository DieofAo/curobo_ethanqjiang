...
# -*- coding: utf-8 -*-
"""
JAKA 7-DOF 工作空间采样器（curobo + GPU 并行 FK）

策略：
  1) 按关节限位以固定步长生成一维网格；笛卡尔积逐"J1 外层切片"流式喂给 FK。
  2) FK 出 (ee_pos, ee_quat)；位置量化成体素索引；四元数取局部 z 轴方向用 HEALPix 量化成姿态桶。
...
def main():
    ...
    ap.add_argument("--ee-link", type=str, default="tool")
    ap.add_argument("--robot", type=str, default="jaka.yml")
    ap.add_argument("--out-dir", type=str,
                    default="/data/workspace/curobo_ethanqjiang/workspace_rviz")
    ...
    if args.collision == "self":
        # RobotWorld 内部会构建自己的 CudaRobotModel; 我们复用它的 km, 不再单独建一个
    ...
    grids = []
    for i in range(dof):
        n_i = int(math.floor((upper[i] - lower[i]) / args.step)) + 1
        g = lower[i] + np.arange(n_i, dtype=np.float32) * args.step
        if g[-1] < upper[i] - 1e-9:
            g = np.concatenate([g, np.array([upper[i]], dtype=np.float32)])
        grids.append(g)
        print(f"  J{i+1}: n={len(g):3d}, range=[{g[0]:+.4f}, {g[-1]:+.4f}]")
